...
def create_tsne_plot(task_vectors):
    ...
    task_vectors_labels = torch.cat(
        [torch.full_like(v[:, 0], i) for i, v in enumerate(task_vectors.values())],
        dim=0,
    )
    ...
    if color_by_task_type:
        ...
        task_type_to_color = {
            task_type: f"C{i}" for i, task_type in enumerate(unique_task_types)
        }
...
def create_histograms_plot(task_vectors):
    ...
    within_task_distances = torch.stack(
        [
            torch.tensor(
                cdist(
                    task_vectors[task_name], task_vectors[task_name], metric="cosine"
                ).flatten()
            )
...
def print_histograms_stats(task_vectors):
    ...
    df["Task"] = df["Task"].apply(
        lambda task_name: task_name.replace("Present Simple", "").strip()
    )
