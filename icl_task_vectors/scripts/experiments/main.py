...
def get_results_file_path(
    model_type: str, model_variant: str, experiment_id: str = ""
) -> str:
    return os.path.join(
        main_experiment_results_dir(experiment_id), f"{model_type}_{model_variant}.pkl"
    )
...
def evaluate_task(
    ...
    predictions = run_icl(
        model, tokenizer, task, baseline_datasets, include_train=False
    )
    accuracies["baseline"] = calculate_accuracy_on_datasets(
        task, predictions, baseline_datasets
    )
    ...
    test_datasets = task.create_datasets(
        num_datasets=num_test_datasets, num_examples=num_examples
    )
    dev_datasets = task.create_datasets(
        num_datasets=num_dev_datasets, num_examples=num_examples
    )
    ...
    accuracies["icl"] = calculate_accuracy_on_datasets(
        task, icl_predictions, test_datasets
    )
    accuracies["tv"] = calculate_accuracy_on_datasets(
        task, tv_predictions, test_datasets
    )
    ...
    try:
        for layer_num in tv_dev_accuracy_by_layer.keys():
            ...
            tv_ordered_tokens_by_layer[layer_num] = logits_top_tokens(
                logits, tokenizer, k=100
            )
...
def run_main_experiment(
    ...
    results_file = get_results_file_path(
        model_type, model_variant, experiment_id=experiment_id
    )
    ...
    for i, task_name in enumerate(TASKS_TO_EVALUATE):
        ...
        tic = time.time()
        accuracies, tv_ordered_tokens_by_layer = evaluate_task(
            model, tokenizer, task_name, num_examples
        )
...
def get_new_experiment_id() -> str:
    return str(
        max(
            [
                int(results_dir)
                for results_dir in os.listdir(MAIN_RESULTS_DIR)
                if results_dir.isdigit()
            ]
            + [0]
        )
        + 1
...
def print_gpu_memory():
    ...
    print(f"Memory Max Allocated: {torch.cuda.max_memory_allocated() / 1e9} GB")
    print(f"Memory Max Cached: {torch.cuda.max_memory_reserved() / 1e9} GB")
...
def main():
    ...
    for model_name, model_fp in MODEL_FP_MAP.items():
        print(f"Before running experiment for {model_name}:")
        print_gpu_memory()
        name_size = model_name.split("-")
        model, tokenizer = load_model_and_tokenizer(model_fp)
        run_main_experiment(
            name_size[0],
            name_size[1],
        ...
        print(f"After running experiment for {model_name}:")
        print_gpu_memory()
        del model, tokenizer
        gc.collect()
        torch.cuda.empty_cache()
        print(f"After clearing memory for {model_name}:")
        print_gpu_memory()
