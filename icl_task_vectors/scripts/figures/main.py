...
def filter_tasks_with_low_icl_accuracy(
    grouped_accuracies_df, regular_accuracy_threshold=0.20
):
    mask = (
        grouped_accuracies_df["Regular"] - grouped_accuracies_df["Baseline"]
    ) >= regular_accuracy_threshold
    ...
    if not mask.all():
        print(
            ...
            grouped_accuracies_df[~mask][["model", "task_name", "Regular"]].apply(
                lambda x: f"({x['model']}, {x['task_name']}): {x['Regular']:.2f}",
                axis=1,
            ),
        )
...
def plot_avg_accuracies_per_model(grouped_accuracies_df):
    filtered_task_accuracies_df = filter_tasks_with_low_icl_accuracy(
    ...
    model_names = sorted(
        model_names, key=lambda x: (x.split(" ")[0], float(x.split(" ")[1][:-1]))
    )
    ...
    legend_elements = [
        Patch(
            facecolor="grey", edgecolor="white", hatch=hatches[0] * 2, label="Baseline"
        ),
        Patch(
            facecolor="green", edgecolor="white", hatch=hatches[2] * 2, label="Regular"
        ),
        Patch(
            facecolor="blue",
            edgecolor="white",
...
def plot_accuracy_by_layer(
    ...
    for idx, model_name in enumerate(model_names):
        min_num_layers = min(
            len(results[model_name][task_name]["tv_dev_accruacy_by_layer"])
        ...
        layers = np.array(
            list(
                list(results[model_name].values())[0]["tv_dev_accruacy_by_layer"].keys()
            )
        )
...
def create_results_latex_table(grouped_accuracies_df):
    ...
    caption_and_label = r"""\caption{Complete results of the main experiment for all tasks and models.} \label{table:main_results} \\"""
...
def create_all_figures(experiment_id: str):
    ...
    plot_accuracy_by_layer(results, model_names=MODEL_DISPLAY_NAME_MAPPING.keys())
    for model_name in MODEL_DISPLAY_NAME_MAPPING.keys():
        plot_accuracy_by_layer(
            results, model_names=[model_name], filename_suffix=model_name
        )
