...
tokenizer_hashes = [
    hash("".join(str(x) for x in sorted(tokenizer.get_vocab().keys())))
    for tokenizer in tokenizers
]
print("Total number of tokenizers:", len(tokenizers))
tokenizers = list(
    {hash: tokenizer for hash, tokenizer in zip(tokenizer_hashes, tokenizers)}.values()
)
...
for tokenizer in tokenizers:
    ...
    for task_name in TASKS_TO_EVALUATE:
        ...
        if (
            task.num_examples() < NUM_EXAMPLES_WARNING
            and "algorithmic" not in task_name
        ):
            bad_tasks.append(
