...
def calculate_accuracy(
    task: Task, predictions: List[str], expected_outputs: List[str]
) -> List[bool]:
...
def calculate_accuracy_on_datasets(
    task: Task, predictions: List[str], datasets: List[FewShotDataset]
) -> List[bool]:
    ...
    if accuracy < 1:
        ...
        column_width = (
            max(
                ...
                for column in zip(headers, inputs, predictions, expected_outputs)
            )
            + 4
        )
        ...
        inputs, predictions, expected_outputs, correct = zip(
            *sorted(
                zip(inputs, predictions, expected_outputs, correct), key=lambda x: x[0]
            )
        )

        print(
            f"{'Input':{column_width}}{'Output':{column_width}}{'Expected':{column_width}}"
        )
        for inp, prediction, expected_output, corr in zip(
            inputs, predictions, expected_outputs, correct
        ):
            if not corr:
                print(
                    f"{inp:{column_width}}{prediction:{column_width}}{expected_output:{column_width}}"
                )
