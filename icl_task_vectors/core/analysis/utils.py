...
def get_token_ids(
    tokenizer: PreTrainedTokenizer, token: str, all_capitalizations: bool = True
):
...
def tokens_ranks(
    logits: torch.Tensor, tokenizer: PreTrainedTokenizer, tokens: List[str]
):
...
def token_ids_ranks(
    logits: torch.Tensor, token_ids: torch.Tensor, keep_token_ids: torch.Tensor = None
):
...
def logits_top_tokens(
    logits: torch.Tensor, tokenizer: PreTrainedTokenizer, k: Optional[int] = None
):
