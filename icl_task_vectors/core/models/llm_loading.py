...
def _get_falcon_device_map() -> Dict[str, int]:
    ...
    device_map.update(
        {
            f"transformer.h.{i}": (i // num_layers_per_device + 1)
...
def _create_device_map(model_path: str) -> Dict[str, int]:
    ...
    base_device_map = infer_auto_device_map(
        model, max_memory=max_memory, no_split_module_classes=[layer_class]
    )
    ...
    device_map_layers = {
        k: v for k, v in base_device_map.items() if k.startswith(layers_path)
    }
    device_map_other = {
        k: v for k, v in base_device_map.items() if k not in device_map_layers
    }
    ...
    num_layers_per_device = math.ceil(num_layers / (num_devices - 1))
    device_map_layers = {
        k: (i // num_layers_per_device + 1) for i, k in enumerate(device_map_layers)
    }
...
def load_model(model_path: str, load_to_cpu: bool = False):
    ...
    #     1: "85GiB",
    # }
    # model = AutoModelForCausalLM.from_pretrained(model_path, device_map='balanced', max_memory=max_memory, **kwargs)
...
def load_model_and_tokenizer(
    model_path: str, load_to_cpu: bool = False
) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:
    tokenizer = load_tokenizer(model_path)
    model = load_model(model_path, load_to_cpu=load_to_cpu)
