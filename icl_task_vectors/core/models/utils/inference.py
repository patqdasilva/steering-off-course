...
def get_input_type(inputs: Dict) -> str:
    ...
    if "input_ids" in inputs and "inputs_embeds" in inputs:
        raise ValueError(
            "inputs must contain either input_ids or inputs_embeds, not both"
        )
...
def _get_batches(
    inputs: Dict, batch_size: int, show_progress: bool = False
) -> Iterable[Dict]:
    ...
    batches = (
        nested_apply(inputs, lambda t: t[i : i + batch_size]) for i in batches_idx
    )
    if show_progress:
...
def _auto_batch_size(model: PreTrainedModel, inputs: Dict) -> int:
    ...
    model_size_gb = sum(get_nested_tensor_size(t) for t in model.parameters()) / (
        1024**3
    )
...
def decode_predictions(
    ...
    answers = [
        tokens.split(few_shot_format.example_separator)[0] for tokens in new_tokens
    ]
    return answers
...
def tokenize_prompts(
    ...
    # Uncomment this when running instr models
    # request = 'Please use the first word in your response to complete this series of examples.\n'
    # prompts = [
...
def traced_forward_context_manager(
    model: PreTrainedModel,
) -> Tuple[ContextManager, ForwardTrace]:
    forward_trace = ForwardTrace()
