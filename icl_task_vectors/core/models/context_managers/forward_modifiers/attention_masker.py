...
class AttentionMasker:
    def __init__(
        self, model: PreTrainedModel, attention_masks: Optional[torch.Tensor] = None
    ):
        self._model = model
    ...
    def _register_forward_hooks(self):
        ...
        def attention_masking_hook(layer_num: int):
            def hook(module, args, kwargs):
                attention_mask = self._attention_masks[layer_num].to(
        ...
        for i, attn in enumerate(self._attn_layers):
            hook = attn.register_forward_pre_hook(
                attention_masking_hook(i), with_kwargs=True
            )
...
def few_shot_attention_masks(
    ...
    example_length = (
        len(tokenizer.tokenize(prompt.split(few_shot_format.example_separator)[0])) + 1
    )
