...
class Task(ABC):
    ...
    @abstractmethod
    def sample_inputs(
        self, num_inputs: int, exclude: Optional[Iterable[Any]] = ()
    ) -> List[Any]:
        pass
    ...
    def compare_outputs(self, output1: Any, output2: Any) -> bool:
        ...
        if self.allow_prefix:
            ...
            return nonempy and (
                output1.startswith(output2) or output2.startswith(output1)
            )
    ...
    def create_datasets(
        self, num_datasets: int, num_examples: int
    ) -> List[FewShotDataset]:
        return [self.create_dataset(num_examples) for _ in range(num_datasets)]

    def create_dataset(
        self, num_examples: int, test_input: Optional[Any] = None
    ) -> FewShotDataset:
