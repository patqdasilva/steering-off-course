...
def _is_single_token(tokenizer: PreTrainedTokenizer, token: str) -> bool:
    return (
        len(tokenizer.tokenize(f"!{token}")) == 2
    )  # this is a hack, might not work for all tokenizers


def filter_single_token_outputs(
    tokenizer: PreTrainedTokenizer, mapping: Dict[str, str]
) -> Dict[str, str]:
...
class MappingTask(Task):
    def __init__(
        ...
        mapping_file = os.path.join(
            config.DATA_DIR, mapping_type, f"{mapping_name}.json"
        )
        ...
        if allow_prefix:
            ...
            filtered_mapping_leading_space = filter_single_token_outputs(
                tokenizer, mapping_leading_space
            )
