...
from core.models.context_managers.forward_modifiers.hidden_injector import (
    HiddenInjector,
)
...
def run_icl(
    ...
    inputs = tokenize_datasets(
        tokenizer, test_datasets, format_dataset_kwargs=format_dataset_kwargs
    ).to("cuda")
    new_ids = batch_generate(
        model, tokenizer, inputs=inputs, generate_kwargs={"max_new_tokens": 1}
    )
...
def run_task_vector(
    ...
    best_intermediate_layer = int(
        max(dev_accuracy_by_layer, key=dev_accuracy_by_layer.get)
    )

    task_hiddens = get_task_hiddens(
        model, tokenizer, task, test_datasets, multi_context=multi_context
    )
...
def run_overriding_task_vector(
    ...
    task_hiddens_datasets = (
        test_datasets if overriding_datasets is None else overriding_datasets
    )
...
def get_multi_context_task_hiddens(
    ...
    for i, dataset in enumerate(datasets):
        for j, other_dataset in enumerate(datasets):
            if (
                dataset.test_input in other_dataset.train_inputs
                or dataset.test_input == other_dataset.test_input
            ):
                mask[i, j] = 0

    task_hiddens = torch.cat(
        [
            task_hiddens[mask[i].bool()].mean(dim=0).unsqueeze(0)
...
def get_single_context_task_hiddens(
    ...
    new_datasets = [
        ...
        for test_input in task.sample_inputs(
            num_test_inputs_to_avg, exclude=(dataset.test_input,)
        )
    ]
    ...
    task_hiddens = task_hiddens.view(
...
def modulated_generate(
    ...
    inputs = tokenize_datasets(
        tokenizer, test_datasets, format_dataset_kwargs={"include_train": include_train}
    )
    ...
    first_predicted_token_ids = (
        first_forward_outputs.logits[:, -1].argmax(dim=-1).unsqueeze(-1)
    )
...
def modulated_forward(
    ...
    if isinstance(intermediate_layer, int):
        intermediate_layer = torch.tensor(intermediate_layer).repeat(
            len(inputs["input_ids"])
        )
    injection_positions = -1 * torch.ones_like(intermediate_layer, dtype=torch.long)
    task_hiddens = task_hiddens[
        torch.arange(len(intermediate_layer)), intermediate_layer
    ]
    ...
    if past_key_values is not None:
        inputs[get_input_type(inputs)] = inputs[get_input_type(inputs)][
            :, -1
        ].unsqueeze(1)
    ...
    first_forward_outputs = modified_forward(
        ...
        batch_size=len(
            inputs["input_ids"]
        ),  # TODO: need to enable batched forward with HiddenInjector
    )
...
def task_vector_accuracy_by_layer(
    ...
    # Get task hiddens
    task_hiddens = get_task_hiddens(
        model, tokenizer, task, datasets, multi_context=multi_context
    )

    # Get input past_key_values
    inputs = tokenize_datasets(
        tokenizer, datasets, format_dataset_kwargs={"include_train": False}
    )
    outputs = batch_forward(model, inputs=inputs, forward_kwargs={"use_cache": True})
    past_key_values = outputs.past_key_values
    past_key_values = nested_apply(
        past_key_values, lambda x: x[:, :, :-1]
    )  # remove last token from past_key_values
    ...
    accuracy_by_layer = {
        layer: accuracy for layer, accuracy in zip(layers_to_test, accuracies)
    }
...
def continue_generation(
    ...
    full_attention_mask = torch.cat(
        [inputs["attention_mask"], new_attention_mask], dim=-1
    )
