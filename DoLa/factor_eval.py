...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-fp", type=str, default="")
    parser.add_argument("--num-gpus", type=str, default="1")
    parser.add_argument("--max_gpu_memory", type=int, default=27)
    parser.add_argument("--device", type=str, choices=["cuda", "cpu"], default="cuda")
    parser.add_argument("--data-path", type=str, default="./factor")
    parser.add_argument("--output-path", type=str, default="./factor_result.json")
    parser.add_argument("--post_softmax", type=str, default="n")
    parser.add_argument("--length_norm", type=str, default="n")
    parser.add_argument("--len_bias_const", type=int, default=0)
    parser.add_argument("--ln_type", type=str, default="next_layer")
    ...
    device = args.device
    post_softmax = True if args.post_softmax == "y" else False
    length_norm = True if args.length_norm == "y" else False
    os.makedirs(os.path.dirname(args.output_path), exist_ok=True)

    print(model_name, model_fp, "FACTOR news")
    print("alpha", args.relative_top, "ln_type", args.ln_type)
    print("post_softmax", post_softmax)
    print("args.early_exit_layers", args.early_exit_layers)
    ...
    if args.parallel:
        ...
        list_data_dict = list_data_dict[
            args.shard_id * chunk_size : (args.shard_id + 1) * chunk_size
        ]

    if args.debug:
        list_data_dict = list_data_dict[:10]

    llm = DoLa(model_name, model_fp, device, num_gpus, args.max_gpu_memory)
    ...
    for sample in tqdm(list_data_dict):
        ...
        answer_true_log_prob, c_dist = llm.lm_score(
            context, answer_true, **generate_kwargs
        )
        if mode == "dola":
        ...
        result_dict["model_completion"].append(
            [answer_true_log_prob] + answer_false_log_probs
        )
