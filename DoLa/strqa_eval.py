...
def clean_answer(model_pred, random_guess=False):
    ...
    if "Thus, yes." in model_pred:
        ...
        print(
            "Warning: answer trigger not found in model prediction:",
            model_pred,
    ...
    if preds not in ["yes", "no"]:
        print(
            "Warning: model prediction is not yes/no:",
            preds,
            "; returning no",
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(fp):
        ...
        with zipfile.ZipFile(
            os.path.join(args.data_path, "strategyqa_dataset.zip"), "r"
        ) as zip_ref:
    ...
    for sample in tqdm(list_data_dict):
        ...
        for i in range(retry_times):
            input_text = build_prompt(
                sample["question"], N_SHOT, COT_FLAG, args.do_shuffle
            )
            ...
            model_answer = clean_answer(
                model_completion, random_guess=(i == retry_times - 1)
            )
