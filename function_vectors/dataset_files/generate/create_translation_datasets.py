...
if __name__ == "__main__":
    ...
    path_exists = [
        os.path.exists(f"./translation/{lang_id}.0-5000.txt")
        for lang_id in d_names.keys()
    ] + [
        os.path.exists(f"./translation/{lang_id}.5000-6500.txt")
        for lang_id in d_names.keys()
    ]

    assert all(
        path_exists
    ), "Original data missing! Please download corresponding 'train' and 'test' files from https://github.com/facebookresearch/MUSE#ground-truth-bilingual-dictionaries in order to re-generate translation_datasets."
    ...
    for lang_id in d_names.keys():
        valid = []
        for d_base in [
            f"./translation/{lang_id}.0-5000.txt",
            f"./translation/{lang_id}.5000-6500.txt",
        ]:
