...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_eval_templates",
        help="Number of templates to evaluate with",
        required=True,
        type=int,
        default=15,
    )
    ...
    parser.add_argument(
        "--seed", help="Randomized seed", type=int, required=False, default=5678
    )
    parser.add_argument(
        "--device",
    ...
    fs_res_dict, zs_res_dict, fs_shuffled_res_dict, templates = portability_eval(
        dataset,
        fv,
        eval_edit_layer,
        model,
    ...
    fs_shuffled_results_file_name = make_valid_path_name(
        f"{save_path_root}/fs_shuffled_port_eval.json"
    )
