...
def natural_text_eval(
    ...
    if loc_data:  # country-capital & similar datasets
        sentences = [
            "A couple years ago I visited {X}, and",
    ...
    for j in range(len(sentences)):
        ...
        for i in tqdm(range(len(filter_set)), total=len(filter_set)):
            ...
            clean_output, fv_output = fv_intervention_natural_text(
                sentence,
                edit_layer,
                fv_vector,
                model,
...
def nattext_main(
    ...
    for dataset_name in datasets:
        ...
        for _ in range(n_seeds):
            ...
            results, clean_results, _ = natural_text_eval(
                dataset,
                fv,
                model,
                model_config,
            ...
            clean_results_dict[dataset_name].append(
                [np.mean(clean_results[i]) for i in clean_results.keys()]
            )
            interv_results_dict[dataset_name].append(
                [np.mean(results[i]) for i in results.keys()]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--edit_layer",
        help="Layer to add function vector to",
        type=int,
        required=False,
        default=9,
    )
    ...
    clean_results_dict, interv_results_dict, seeds_dict = nattext_main(
        datasets,
        model,
        model_config,
        tokenizer,
    ...
    with open(
        os.path.join(save_path_root, "nattext_eval_results.txt"), "w"
    ) as out_file:
        for d in datasets:
            ...
            print(
                "clean results:",
                clean_acc.round(3) * 100,
                "% +/-",
                clean_std.round(3) * 100,
                file=out_file,
            )
            print(
                "fv results:",
