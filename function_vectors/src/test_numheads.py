...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_name", help="dataset to be evaluated", type=str, required=True
    )
    parser.add_argument(
        "--mean_act_root",
        help="root path to mean activations",
        type=str,
        required=False,
        default="IE_template_QA/gptj",
    )
    parser.add_argument(
        "--model_name", type=str, required=True, default="EleutherAI/gpt-j-6b"
    )
    parser.add_argument("--model_nickname", type=str, required=False, default="gptj")
    parser.add_argument(
        "--n_heads",
        type=int,
        help="upper bound of the number of heads to create the FV",
        required=True,
        default=40,
    )
    parser.add_argument(
        "--edit_layer",
        type=int,
        help="layer at which to add the function vector",
        required=True,
        default=9,
    )
    parser.add_argument("--seed", required=False, type=int, default=42)
    parser.add_argument(
        "--save_path_root", required=True, type=str, default="../results"
    )
    ...
    mean_activations = torch.load(
        f"{save_path_root}/{mean_act_root}/{dataset_name}/{dataset_name}_mean_head_activations.pt"
    )
    ...
    for i in range(n_heads + 1):
        fv, _ = compute_universal_function_vector(
            mean_activations, model, model_config, i
        )
        zs_results[i] = n_shot_eval(
            dataset,
            fv,
            edit_layer,
            0,
    ...
    json.dump(
        zs_results,
        open(
            f"{save_path_root}/{model_nickname}_test_numheads/{dataset_name}_perf_v_heads.json",
            "w",
        ),
    )
