...
def compute_best_token_rank(prob_dist, target_ids) -> int:
    ...
    """
    related_token_ranks = [
        compute_individual_token_rank(prob_dist, x) for x in target_ids
    ]
...
def decode_to_vocab(prob_dist, tokenizer, k=5) -> list:
    ...
    return [
        ...
        for x, y in zip(
            get_topk(prob_dist, k).indices[0], get_topk(prob_dist, k).values[0]
        )
    ]
...
def fv_to_vocab(function_vector, model, model_config, tokenizer, n_tokens=10):
    ...
    if "gpt-j" in model_config["name_or_path"]:
        decoder = torch.nn.Sequential(
            model.transformer.ln_f, model.lm_head, torch.nn.Softmax(dim=-1)
        )
    elif "llama" in model_config["name_or_path"]:
        decoder = torch.nn.Sequential(
            model.model.norm, model.lm_head, torch.nn.Softmax(dim=-1)
        )
    else:
        raise ValueError("Model not yet supported")

    d_out = decoder(
        function_vector.reshape(1, 1, model_config["resid_dim"]).to(model.device)
    )

    vals, inds = torch.topk(d_out, k=n_tokens, largest=True)
    decoded_tokens = [
        (tokenizer.decode(x), round(y.item(), 4))
        for x, y in zip(inds.squeeze(), vals.squeeze())
    ]
...
def sentence_eval(
    ...
    if compute_nll:
        ...
        nll_targets[:, :-target_len] = (
            -100
        )  # This is the accepted value to skip indices when computing loss in nn.CrossEntropyLoss
...
def n_shot_eval(
    ...
    prepend_bos = False  # False if model_config['prepend_bos'] else True
    ...
    for j in range(
        ...
        if generate_str:
            if metric == "f1_score":
                ...
                raise ValueError(
                    f'Unknown metric: {metric}. Recognized metrics: ["f1_score", "exact_match_score"]'
                )
            ...
            clean_parsed_str, clean_score = parse_generation(
                clean_output, target, metric_fn
            )
            intervention_parsed_str, intervention_score = parse_generation(
                intervention_output, target, metric_fn
            )
            ...
            hv_norms1.append(norm_hv1)
            hv_norms2.append(norm_hv2)
            hv_std.append(std_hv)
            hv_ms.append(ms_hv)
            hv_spar.append(spar_hv)
            fv_norms.append(norm_fv)
            clean_rank = compute_individual_token_rank(clean_output, target_token_id)
            intervention_rank = compute_individual_token_rank(
                intervention_output, target_token_id
            )
    ...
    if generate_str:
        ...
        results = {
            "clean_topk": [
                (K, compute_top_k_accuracy(clean_rank_list, K)) for K in range(1, 4)
            ],
            "clean_rank_list": clean_rank_list,
            "intervention_topk": [
                (K, compute_top_k_accuracy(intervention_rank_list, K))
                for K in range(1, 4)
            ],
    ...
    return results, (hv_norms1, hv_norms2, hv_std, hv_ms, hv_spar, fv_norms)
...
def n_shot_eval_no_intervention(
    ...
    for j in tqdm(range(len(dataset[test_split])), total=len(dataset[test_split])):
        ...
        if compute_ppl:
            clean_output, clean_nll = sentence_eval(
                sentence,
                target=[target],
                model=model,
                tokenizer=tokenizer,
...
def portability_eval(
    ...
    random_combos = [
        ...
        for x in np.array(all_combinations)[
            np.random.choice(
                np.arange(len(all_combinations)), n_eval_templates, replace=False
            )
        ]
    ]
