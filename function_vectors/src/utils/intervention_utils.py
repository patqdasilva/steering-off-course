    ...
    def rep_act(output, layer_name, inputs):
        ...
        if current_layer in edit_layers:
            ...
            new_shape = inputs.size()[:-1] + (
                ...
                model_config["attn_dim"] // model_config["n_heads"],
            )  # split by head: + (n_attn_heads, hidden_size/n_attn_heads)
            inputs = inputs.view(
                *new_shape
            )  # inputs shape: (batch_size , tokens (n), heads, hidden_dim)
            ...
            if batched_input:
                ...
                for i in range(model_config["n_heads"]):
                    ...
                    inputs[i, token_n, head_n] = avg_activations[
                        layer, head_n, idx_map[token_n]
                    ]
                ...
                for layer, head_n, token_n in layer_head_token_pairs:
                    if layer == current_layer:
                        inputs[-1, -1, head_n] = avg_activations[
                            layer, head_n, idx_map[token_n]
                        ]
            else:
                ...
                for layer, head_n, token_n in layer_head_token_pairs:
                    if layer == current_layer:
                        inputs[-1, token_n, head_n] = avg_activations[
                            layer, head_n, idx_map[token_n]
                        ]
...
def add_function_vector(edit_layer, fv_vector, device, idx=-1):
    ...
    """
    # Create a list to store the hidden activations
    captured_activations = None

    def add_act(output, layer_name):
        current_layer = int(layer_name.split(".")[2])
        if current_layer == edit_layer:
            # Capture hidden activations for the specific layer
            nonlocal captured_activations
            captured_activations = output[0][:, idx].to("cuda:0").clone().detach()
            if isinstance(output, tuple):
                device = output[0][:, idx].device
    ...
    return add_act, lambda: captured_activations
...
def function_vector_intervention(
    ...
    if compute_nll:
        ...
        nll_inputs = tokenizer(target_completion, return_tensors="pt").to("cuda:0")
        nll_targets = nll_inputs.input_ids.clone()
        target_len = len(nll_targets.squeeze()) - len(inputs.input_ids.squeeze())
        nll_targets[:, :-target_len] = (
            -100
        )  # This is the accepted value to skip indices when computing loss (see nn.CrossEntropyLoss default)
        ...
        clean_output = model(**inputs).logits[:, -1, :].to("cuda:0")
        # print('clean_output', clean_output)
        intervention_idx = -1

    # Perform Intervention
    intervention_fn, get_activation = add_function_vector(
        edit_layer,
    ...
    with TraceDict(
        ...
        if compute_nll:
            ...
            intervention_output = (
                model(**inputs).logits[:, -1, :].to("cuda:0")
    ...
    # compute the norm of the hidden activations for the layer of interest
    hidden_activations = get_activation()
    if hidden_activations is not None:
        # print('function_vector.shape', function_vector.shape)
        n = torch.tensor(hidden_activations.shape[1])
        # print('n.item()', n.item())
        # print('torch.sqrt(n).item()', torch.sqrt(n).item())
        norm_hv1 = torch.norm(hidden_activations, p=1).item()  # /n.item()  # L1 norm
        norm_hv2 = torch.norm(
            hidden_activations, p=2
        ).item()  # /torch.sqrt(n).item()  # L2 norm
        std_hv = torch.std(hidden_activations).item()  # std
        # print('torch.median(hidden_activations).item()', torch.median(hidden_activations).item())
        safe_median = torch.median(torch.abs(hidden_activations)).item()
        # max sparsity from https://blog.allenai.org/investigating-pretraining-dynamics-and-stability-with-olmo-checkpoints-ece6f0c4947a
        ms_hv = (
            torch.max(torch.abs(hidden_activations)).item() - safe_median
        ) / safe_median
        spar_hv = torch.sqrt(n).item() * (norm_hv2 / norm_hv1)
        norm_fv = torch.norm(
            function_vector, p=2
        ).item()  # /torch.sqrt(n).item()  # L2 norm
        norms = (norm_hv1, norm_hv2, std_hv, ms_hv, spar_hv, norm_fv)
        # print(f"Norm of hidden activations at layer {edit_layer}: {norm_hv.item()}")
        # print(f"Norm of function vector at layer {edit_layer}: {norm_fv.item()}")

    return fvi_output, norms
