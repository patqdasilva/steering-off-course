...
def gather_attn_activations(
    prompt_data, layers, dummy_labels, model, tokenizer, model_config
):
    """
...
def get_mean_head_activations(
    ...
    if prefixes is not None and separators is not None:
        ...
        dummy_labels = get_dummy_token_labels(
            n_icl_examples, tokenizer=tokenizer, model_config=model_config
        )
    ...
    for n in range(N_TRIALS):
        ...
        word_pairs_test = dataset["valid"][
            np.random.choice(filter_set, n_test_examples, replace=False)
        ]
        if prefixes is not None and separators is not None:
        ...
        stack_initial = torch.vstack(
            [
                split_activations_by_head(
                    activations_td[layer].input.to("cuda:0"), model_config
                )
...
def get_mean_layer_activations(
    dataset,
    model,
    model_config,
    tokenizer,
    ...
    n_test_examples = 1
    activation_storage = torch.zeros(
        N_TRIALS, model_config["n_layers"], model_config["resid_dim"]
    )
    ...
    for n in range(N_TRIALS):
        ...
        activations_td = gather_layer_activations(
            prompt_data=prompt_data,
        ...
        stack_initial = torch.vstack(
            [
                activations_td[layer].output[0].to("cuda:0")
                for layer in model_config["layer_hook_names"]
            ]
        )
...
def get_value_weighted_attention(sentence, model, model_config, tokenizer):
    ...
    attentions = torch.vstack(
        output.attentions.to("cuda:0")
    )  # (layers, heads, tokens, tokens)
    values = torch.vstack(
        [
            output.past_key_values[i][1].to("cuda:0")
            for i in range(model_config["n_layers"])
        ]
    )  # (layers, heads, tokens, head_dim)
    value_weighted_attn = torch.einsum(
        "abcd,abd->abcd", attentions, values.norm(dim=-1)
    )
    return attentions, value_weighted_attn


def get_token_averaged_attention(
    dataset,
    model,
    model_config,
    tokenizer,
    ...
    dummy_labels = get_dummy_token_labels(
        n_shots, tokenizer=tokenizer, model_config=model_config
    )
    attn_storage = torch.zeros(
        storage_size,
        model_config["n_layers"],
        model_config["n_heads"],
        len(dummy_labels),
    ...
    for ind, s in enumerate(storage_inds):
        if n_shots == 0:
            ...
            word_pairs = dataset["train"][
                np.random.choice(len(dataset["train"]), n_shots, replace=False)
            ]
        ...
        prompt_data = word_pairs_to_prompt_data(
            word_pairs, query_target_pair=word_pairs_test, prepend_bos_token=add_bos
        )
        ...
        attentions, value_weighted_attn = get_value_weighted_attention(
            sentence, model, model_config, tokenizer
        )
        ...
        token_avgd_attention = attentionsv2[:, :, list(idx_map.keys())][
            :, :, :, list(idx_map.keys())
        ]
        token_avgd_vw_attention = vw_v2[:, :, list(idx_map.keys())][
            :, :, :, list(idx_map.keys())
        ]
...
def prefix_matching_score(
    ...
    rand_tokens_repeat = torch.concat((rand_tokens, rand_tokens), dim=1).to(
        model.device
    )
    ...
    attentions = torch.vstack(
        output.attentions.to("cuda:0")
    )  # (n_layers, n_heads, tokens, tokens)

    score = attentions.diagonal(1 - seq_len, dim1=-2, dim2=-1)
    score_per_head = (
        score.reshape(model_config["n_layers"], batch_size, model_config["n_heads"], -1)
        .mean(axis=-1)
        .mean(axis=1)
        .cpu()
        .T
...
def compute_function_vector(
    ...
    model_head_dim = model_config["attn_dim"] // model_config["n_heads"]
    device = model.device
    ...
    for L, H, _ in top_heads:
        if any(x in model_config["name"] for x in ["falcon-7b", "gptj"]):
            out_proj = model.transformer.h[L].attn.out_proj
        elif "pythia" in model_config["name"]:
            out_proj = model.gpt_neox.layers[L].attention.dense
        else:
            out_proj = model.model.layers[L].self_attn.o_proj

        x = torch.zeros(model_config["attn_dim"]).to("cuda:0")
        x[H * model_head_dim : (H + 1) * model_head_dim] = mean_activations[L, H, T].to(
            "cuda:0"
        )
        d_out = out_proj(x.reshape(1, 1, model_config["attn_dim"]).to(model.dtype)).to(
            "cuda:0"
        )
...
def compute_universal_function_vector(
    mean_activations, model, model_config, n_top_heads=10
):
    """
    Computes a "function vector" vector that communicates the task observed in ICL examples used for downstream intervention
