...
def tokenize_labels(sentence_parts, text_labels, tokenizer, prepend_bos=False):
    ...
    if prepend_bos:
        labels = extend_labels(
            sentence_parts, text_labels, tokenizer, label_init=["bos_token"]
        )
    else:
...
def get_token_meta_labels(prompt_data, tokenizer, query=None, prepend_bos=False):
    ...
    prompt_parts, prompt_part_labels = get_prompt_parts_and_labels(
        prompt_data, query_sentence=query
    )
    token_meta_labels = tokenize_labels(
        prompt_parts, prompt_part_labels, tokenizer, prepend_bos
    )
...
def get_dummy_token_labels(
    n_icl_examples, tokenizer, model_config, prefixes=None, separators=None
):
    """
...
def compute_duplicated_labels(token_labels, gt_labels):
    ...
    dup_inds = pd.DataFrame(check_inds)[pd.DataFrame(check_inds)[2].duplicated()][
        0
    ].values
    ...
    index_map = {
        k: v[0]
        for (k, v) in zip(
            [x[0] for x in token_labels if x[0] not in dup_inds], gt_labels
        )
    }
...
def word_pairs_to_prompt_data(
    ...
    if prepend_bos_token:
        prefixes = {
            k: (v if k != "instructions" else "" + v) for (k, v) in prefixes.items()
        }
    ...
    if shuffle_labels:
        ...
        if prepend_space:
            ...
            prompt_data["query_target"] = (
                {k: " " + v for k, v in query_target_pair.items()}
                if query_target_pair is not None
                else None
            )
        else:
...
class ICLDataset:
    ...
    def __getitem__(self, i):
        if isinstance(i, int):
            ...
            raise ValueError(
                f"{i} is not a valid index type. Expected one of: [int, list, np.ndarray, slice, str]"
            )
...
def split_icl_dataset(
    dataset, train_size=None, test_size=0.3, seed=42
) -> Dict[str, ICLDataset]:
    """
    ...
    train, valid = train_test_split(
        dataset.raw_data, test_size=test_size, random_state=seed
    )
...
def load_dataset(
    task_name: str, root_data_dir: str = "../dataset_files", test_size=0.3, seed=32
) -> Dict[str, ICLDataset]:
    """
    ...
    d_group_map = [
        (
            dataset_type,
            os.path.exists(
                os.path.join(root_data_dir, dataset_type, task_name + ".json")
            ),
        )
    ...
    assert (
        len(d_group) != 0 and len(d_group) == 1
