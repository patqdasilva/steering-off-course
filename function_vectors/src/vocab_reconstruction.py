...
def vocab_reconstruction(
    ...
    for dataset_name in datasets:
        ...
        for i in range(n_seeds):
            ...
            filter_set_validation = np.where(
                np.array(fs_results_validation["clean_rank_list"]) == 0
            )[0]
            ...
            fv, _ = compute_universal_function_vector(
                mean_activations, model, model_config
            )
            ...
            for j, vocab_size in enumerate(restrict_vocab_list):
                ...
                decoder = torch.nn.Sequential(model.transformer.ln_f, model.lm_head).to(
                    model.device
                )
                ...
                scaled_vn = (
                    computed_vn / torch.linalg.norm(computed_vn) * torch.linalg.norm(fv)
                )

                zs_reconstruction_results = n_shot_eval(
                    dataset=dataset,
                    fv_vector=scaled_vn,
                    edit_layer=9,
                    n_shots=0,
                ...
                if vocab_size != 50400:
                    ...
                    vn_ps = torch.softmax(decoder(vns[dataset_name][j][i]), dim=-1)[
                        :, inds
                    ]
                    ...
                    log_probs = torch.log(
                        torch.softmax(decoder(vns[dataset_name][j][i]), dim=-1)
                    )
                    target_probs = torch.softmax(decoder(fvs[dataset_name][i]), dim=-1)

                kl_divs[dataset_name][j].append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path_root",
        help="File path to save mean activations to",
        type=str,
        required=False,
        default="../results",
    )
    parser.add_argument(
        "--n_seeds", help="Number of seeds", type=int, required=False, default=5
    )
    parser.add_argument(
        "--n_trials",
        help="Number of trials to use for computing task-conditioned mean head activations",
        type=int,
        required=False,
        default=100,
    )
    parser.add_argument(
        "--n_shots",
        help="Number of shots to use for prompts when computing task-conditioned mean head activations",
        type=int,
        required=False,
        default=10,
    )
    ...
    # Test Loop:
    orig_results, zs_results, kl_divs = vocab_reconstruction(
        datasets,
        n_steps=n_steps,
        lr=lr,
        n_seeds=n_seeds,
    ...
    with open(
        os.path.join(save_path_root, "reconstruction_results.txt"), "w"
    ) as out_file:

        for dataset_name in datasets:
            print(f"{dataset_name.title()}:", file=out_file)
            fv_acc = [
                orig_results[dataset_name][i]["intervention_topk"][0][1]
                for i in range(n_seeds)
            ]
            v100_acc = [
                zs_results[dataset_name][0][i]["intervention_topk"][0][1]
                for i in range(n_seeds)
            ]
            kl100_val = kl_divs[dataset_name][0]

            vfull_acc = [
                zs_results[dataset_name][1][i]["intervention_topk"][0][1]
                for i in range(n_seeds)
            ]
            ...
            print(
                "fv results:",
                np.mean(fv_acc).round(3) * 100,
                "% +/-",
                np.std(fv_acc).round(3) * 100,
                file=out_file,
            )
            print(
                "v_100 results:",
                np.mean(v100_acc).round(3) * 100,
                "% +/-",
                np.std(v100_acc).round(3) * 100,
                file=out_file,
            )
            print(
                "KL100:",
                np.mean(kl100_val).round(5),
                "+/-",
                np.std(kl100_val).round(5),
                file=out_file,
            )
            print(
                "v_full results:",
                np.mean(vfull_acc).round(3) * 100,
                "% +/-",
                np.std(vfull_acc).round(3) * 100,
                file=out_file,
            )
            print(
                "KLFull:",
                np.mean(klfull_val).round(5),
                "+/-",
                np.std(klfull_val).round(5),
                "\n",
                file=out_file,
            )
