...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_trials",
        help="Number of in-context prompts to average over for indirect_effect",
        type=int,
        required=False,
        default=25,
    )
    ...
    ie_path_root = (
        f"{args.ie_path_root}/{dataset_name}" if args.ie_path_root else save_path_root
    )
    seed = args.seed
    device = args.device
    ...
    if os.path.exists(fs_results_file_name):
        ...
        filter_set_validation = np.where(np.array(fs_results_validation["score"]) == 1)[
            0
        ]
        set_seed(seed)
    ...
    if indirect_effect_path is not None and os.path.exists(indirect_effect_path):
    ...
    elif indirect_effect_path is None and os.path.exists(
        f"{ie_path_root}/{dataset_name}_indirect_effect.pt"
    ):
        ...
        args.indirect_effect_path = (
            f"{save_path_root}/{dataset_name}_indirect_effect.pt"
        )
    ...
    if universal_set:
        fv, top_heads = compute_universal_function_vector(
            mean_activations, model, model_config=model_config, n_top_heads=n_top_heads
        )
    else:
    ...
    zs_results_file_name = make_valid_path_name(
        f"{save_path_root}/zs_results" + zs_results_file_suffix
    )
    ...
    if compute_baseline:
        ...
        baseline_file_name = make_valid_path_name(
            f"{save_path_root}/model_baseline.json"
        )
