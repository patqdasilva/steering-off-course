...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        mean_activations = get_mean_layer_activations(
            dataset,
            model=model,
        ...
        torch.save(
            mean_activations,
            f"{save_path_root}/{dataset_name}_mean_layer_activations.pt",
        )
        ...
        for i in range(model_config["n_layers"]):
            zs_res[i] = n_shot_eval(
                dataset,
                mean_activations[i].unsqueeze(0),
                i,
                0,
                model,
                model_config,
        ...
        with open(
            f"{save_path_root}/mean_layer_intervention_zs_results_sweep_{seed}.json",
            "w",
        ) as interv_zsres_file:
            json.dump(zs_res, interv_zsres_file, indent=2)
        with open(
            f"{save_path_root}/mean_layer_intervention_fss_results_sweep_{seed}.json",
            "w",
        ) as interv_fssres_file:
