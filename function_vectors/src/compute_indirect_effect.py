...
def activation_replacement_per_class_intervention(
    ...
    # Compute causal effect for all token classes (instead of just last token)
        ...
        token_classes_regex = [
            "demonstration_[\d]{1,}_token",
            "demonstration_[\d]{1,}_label_token",
            "separator_token",
            "predictive_token",
            "structural_token",
    ...
    indirect_effect_storage = torch.zeros(
        model_config["n_layers"], model_config["n_heads"], len(token_classes)
    )

    # Clean Run of Baseline:
    clean_output = model(**inputs).logits[:, -1, :].to("cuda:0").detach()
    ...
    for layer in range(model_config["n_layers"]):
        ...
        for head_n in range(model_config["n_heads"]):
            for i, (token_class, class_regex) in enumerate(
                zip(token_classes, token_classes_regex)
            ):
                ...
                with TraceDict(
                    model, layers=head_hook_layer, edit_output=intervention_fn
                ) as td:
                    output = (
                ...
                indirect_effect_storage[layer, head_n, i] = (
                    ...
                    .index_select(
                        1, torch.LongTensor(token_id_of_interest).to("cuda:0").squeeze()
                    )
                    .squeeze()
                )
...
def compute_indirect_effect(
    ...
    if last_token_only:
        indirect_effect = torch.zeros(
            n_trials, model_config["n_layers"], model_config["n_heads"]
        )
    else:
        indirect_effect = torch.zeros(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", help="Randomized seed", type=int, required=False, default=42
    )
    parser.add_argument(
        "--n_shots",
    ...
    # Don't rerun if already complete
    if os.path.isfile(f"{save_path_root}/{dataset_name}_indirect_effect.pt"):
        print(
            f"Model: {model_name}, already ran for Dataset: {dataset_name}", flush=True
        )
        import sys
    ...
    model, tokenizer, model_config = load_gpt_model_and_tokenizer(
        model_fp, model_name, device=device
    )
    ...
    dataset = load_dataset(
        dataset_name, root_data_dir=root_data_dir, test_size=test_split, seed=seed
    )
